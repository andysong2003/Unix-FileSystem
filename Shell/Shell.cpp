...
void Shell::mkdir()
{
    ...
    if (getParamAmount() == 2)
    {
        InodeId result = bounded_VFS->mkDir(getParam(1));
        if (result < 0)
        {
            switch (result)
            {
            ...
            case ERROR_PERMISSION_DENIED:
                Logcat::log("ERROR!权限不足，无法在当前目录创建目录！");
                break;
            case ERROR_NOTSPEC:
                Logcat::log("ERROR!目录空间不足，创建失败！");
                break;
            default:
                Logcat::log("ERROR!目录创建失败！");
                break;
            }
        }
        else
...
void Shell::touch()
{
    ...
    {
        InodeId result = bounded_VFS->createFile(getParam(1));
        if (result < 0)
        {
            switch (result)
            {
            case ERROR_PERMISSION_DENIED:
                Logcat::log("ERROR!权限不足，无法在当前目录创建文件！");
                break;   
            case ERROR_FILENAME_EXSIST:
...
void Shell::rm()
{
    ...
    {
        int result = bounded_VFS->deleteFile(getParam(1));
        if (result < 0)
        {
            switch (result)
            {
            case ERROR_PERMISSION_DENIED:
                Logcat::log("ERROR!权限不足，无法删除文件！");
                break;
            case ERROR_FILE_NOT_FOUND:  // 如果有定义文件不存在的错误码
                Logcat::log("ERROR!文件不存在！");
                break;
            default:
                Logcat::log("ERROR!删除文件失败！");
                break;
            }
        }
        else
...
void Shell::rmdir()
{
    ...
    {
        int result = bounded_VFS->deleteDir(getParam(1));
        if (result < 0)
        {
            switch (result)
            {
            case ERROR_PERMISSION_DENIED:
                Logcat::log("ERROR!权限不足，无法删除目录！");
                break;
            case ERROR_DIR_NOT_FOUND:  // 如果有定义目录不存在的错误码
                Logcat::log("ERROR!目录不存在！");
                break;
            case ERROR_DIR_NOT_EMPTY:  // 如果有定义目录非空的错误码
                Logcat::log("ERROR!目录非空，无法删除！");
                break;
            default:
                Logcat::log("ERROR!删除目录失败！");
                break;
            }
        }
        else
